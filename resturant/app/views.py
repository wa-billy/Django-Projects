...
from app.models import BookTable, AboutUs, Feedback, ItemList, Items, Cart
...
def HomeView(request):
    ...
    return render(request, 'home.html', {'items': items, 'list': lists, 'review': review})
...
def BookTableView(request):
    ...
    if request.method == 'POST':
        ...
        if name != '' and len(phone_number) == 10 and email != '' and total_person != '0' and booking_data != '':
            ...
            message = f"Hello {name}, \n\nYour booking has been successfully recieved.\n"\
...
def SignupView(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Error during sigup. Please try again.')
    else:
